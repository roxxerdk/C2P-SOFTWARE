...
from rag_engine import rag_engine, write_plan_to_memory
from agents.pipeline import run_full_pipeline
import asyncio
from agents.perception_agent import perception_agent
from agents.ballooning_agent import ballooning_agent
...
async def run_pipeline(file: UploadFile = File(...)):
    """Run pipeline in background thread with clean bytes and keep-alives."""
    filename = file.filename
    # Explicitly ensure we get raw bytes
    ...
    async def event_stream():
        ...
        yield f"data: {json.dumps({'stage': 'init', 'status': 'started', 'message': 'Processing drawing...'})}\n\n"

        # 2. Run pipeline in background executor with guaranteed raw bytes
        loop = asyncio.get_running_loop()
        future = loop.run_in_executor(executor, run_full_pipeline, image_bytes, filename)

        # 3. Stream keep-alives while waiting
        while not future.done():
            yield ": keep-alive\n\n"
            await asyncio.sleep(1)

        # 4. Stream results on completion
        try:
            result = future.result()

            for stage_name in ["perception", "ballooning", "memory", "expert", "validation", "documentation"]:
                if stage_name == "memory":
                    payload = result.get("memory_context")
                elif stage_name == "expert":
                    payload = result.get("plan")
                elif stage_name == "documentation":
                    payload = result.get("report")
                else:
                    payload = result.get(stage_name)

                yield f"data: {json.dumps({'stage': stage_name, 'status': 'done', 'data': payload})}\n\n"

            # Memory store step
            try:
                write_plan_to_memory(
                    result.get("drawing_data", result.get("ballooning", {})),
                    result.get("validation", {}).get("optimized_process_plan", result.get("plan", {}).get("process_plan", [])),
                    result.get("validation", {}).get("warnings", []),
                )
                yield f"data: {json.dumps({'stage': 'memory_store', 'status': 'done', 'data': {'source': 'generated'}})}\n\n"
            except Exception as e:
                yield f"data: {json.dumps({'stage': 'memory_store', 'status': 'error', 'error': str(e)})}\n\n"

            yield f"data: {json.dumps({'stage': 'complete', 'status': 'done', 'data': result})}\n\n"
...
# legacy — superseded by /run-pipeline
